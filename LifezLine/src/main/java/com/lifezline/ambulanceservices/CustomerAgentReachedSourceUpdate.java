...
public class CustomerAgentReachedSourceUpdate {

	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(CustomerAgentReachedSourceUpdate.class.getName());

	GenericResponse<TreeMap<String, String>> response1 = new GenericResponse<TreeMap<String, String>>();

	@RequestMapping(method = RequestMethod.POST, value = "/CustomerAgentReachedSourceUpdate")
	@ResponseBody
		...
		try {
			...
			if (!query.list().isEmpty()) {
				...
				while (it.hasNext()) {
					...
					String agent_reached_source = detail.getAgent_reached_source() == null ? ""
							: detail.getAgent_reached_source().toString().trim();

					if (!agent_reached_source.isEmpty()) {
